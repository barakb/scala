package com.gigaspaces.par.noneblocking

import java.util.concurrent.{Callable, CountDownLatch, ExecutorService}

import com.gigaspaces.par.Actor
...
object Par {
  ...
  trait Future[+A] {
    private[noneblocking] def apply(k: A => Unit): Unit
  }
  ...
  object Par {
    ...
    implicit class ParOps[A](val p: Par[A]) extends AnyVal{
      ...
      def zip[B](b: Par[B]): Par[(A, B)] = Par.map2(p, b)((_, _))
    }
